...
from planetarium.serializers import ShowThemeSerializer, PlanetariumDomeListSerializer, PlanetariumDomeCreateSerializer
from planetarium.tests.default_test_data import user_test, sample_show_theme, sample_planetarium_dome
...
class AuthenticatedShowThemeApiTests(TestCase):
    ...
    def test_list_planetarium_dome(self):
        ...
        sample_planetarium_dome(name="Planetarium Dome Test", rows=40, seats_in_row=200)
        res = self.client.get(Planetarium_Dome_URL)
        planetarium_dome = PlanetariumDome.objects.all()
        serializer = PlanetariumDomeListSerializer(planetarium_dome, many=True)
...
class PlanetariumDomeFilteringApiTests(TestCase):
    ...
    def test_filter_planetarium_dome_name(self):
        planetarium_dome_1 = sample_planetarium_dome(name="Planetarium Dome Test1", rows=40, seats_in_row=200)

        planetarium_dome_2 = sample_planetarium_dome(name="Planetarium Dome Test2", rows=40, seats_in_row=200)

        planetarium_dome_3 = sample_planetarium_dome(name="No Match", rows=40, seats_in_row=200)

        res = self.client.get(Planetarium_Dome_URL, {"planetarium_name": "planetarium"})
    ...
    def test_filter_planetarium_rows(self):
        ...
        planetarium_dome_3 = sample_planetarium_dome(name="Planetarium Dome Test3", rows=35, seats_in_row=200)
    ...
    def test_filter_planetarium_seats_rows(self):
        ...
        planetarium_dome_3 = sample_planetarium_dome(name="Planetarium Dome Test3", rows=35, seats_in_row=150)
...
class PlanetariumValidateTest(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.test_user = user_test(username="admin", password="testadmin26", is_staff=True)
        self.client.force_authenticate(self.test_user)

    def test_planetarium_validate_serializers_name(self):
        with self.assertRaises(ValidationError):
            payload1 = {"name": "Planetarium Dome Test 1", "rows": 40, "seats_in_row": 200}
            serializer1 = PlanetariumDomeCreateSerializer(data=payload1)
            serializer1.is_valid()
            serializer1.save()
            payload2 = {"name": "Planetarium Dome Test 1", "rows": 40, "seats_in_row": 200}
    ...
    def test_planetarium_validate_serializers_rows(self):
        with self.assertRaises(ValueError):
            sample_planetarium_dome(name="Planetarium Dome Test1", rows=0, seats_in_row=200)

        with self.assertRaises(ValueError):
            sample_planetarium_dome(name="Planetarium Dome Test2", rows=51, seats_in_row=200)

        with self.assertRaises(ValueError):
            sample_planetarium_dome(name="Planetarium Dome Test3", rows=50, seats_in_row=631)
...
class PlanetariumModelsTests(TestCase):
    ...
    def test_show_theme_str(self):
        planetarium_dome = sample_planetarium_dome(name="Test1", rows=40, seats_in_row=200)
        self.assertEqual(planetarium_dome.name.__str__(), "Test1")
        self.assertEqual(planetarium_dome.__str__(),
